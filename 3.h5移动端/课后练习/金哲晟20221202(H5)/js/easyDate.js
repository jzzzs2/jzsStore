(function (w) {
  const DEFAULT_FORMAT = "yyyy-mm-dd";
    class EasyDate {
      ...
      toObject (today) {
        console.log(today);
        console.log(!today ? new Date(this.base):undefined);
        console.log(this.base);
        return {
          year: this.base.getFullYear(),
          month: this.base.getMonth() + 1,
          empty: this.getFirstDayOfThisMonth(),
          days: EasyDate.getDates(this.base,!today ? new Date(this.base):undefined)
        }
      }
      toPreObject () {
        return {
          ...
          month: this.base.getMonth() + 1,
          empty: this.getFirstDayOfThisMonth(),
          days: EasyDate.getPreDates(this.base)
        }
      }
      /**
      ...
      static getDates(date, today, start, end) {
        ...
        while (newDate.getMonth() === month) {
          ...
          console.log((start ? easyDate.toString() > start?.toString() : true),(end ? easyDate.toString() < end?.toString() : true),(easyDate.toString() > today?.toString()));
          let obj = {
            disabled: (start ? easyDate.toString() > start?.toString() : true) && (end ? easyDate.toString() < end?.toString() : true) && (today === null ? true :easyDate.toString() > today?.toString()),
       ...
       * @param {String} str
       * @return {*} 将字符串拆分 改变当月
       * @Date: 2022-12-02 13:01:32
       */     
      static transDate (str,date) {
        let reg = /([+-]\d+)([ymd])/;
        let result = str.match(reg);
        console.log(result);
        console.log(date);
        console.log(`set${timeMap[result[2]]}`);
        console.log(Number(result[1]));
        date[`set${timeMap[result[2]]}`](date[`get${timeMap[result[2]]}`]() + Number(result[1]));
        // console.log(date);
      }
    }
  w.EasyDate = EasyDate
