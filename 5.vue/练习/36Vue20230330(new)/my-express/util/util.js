 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2023-03-20 15:29:21
 */
const pagination = require("mongoose-sex-page")
async function pagiUtil (model,{size = 100,page = 1,display = 10,query={}} = {},link={}) {
  console.log(size,page,"show size page");
  let result = await pagination(model).find(query).populate(link).select("+password").sort("-createtime").page(page).size(size).display(display).exec()
