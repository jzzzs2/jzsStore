 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2023-03-24 20:27:38
 */
...
route.get("/", async (req, res, next) => {
  ...
  console.log("获得所有分类",query,typeof query);
  let result
  // if (query) {
  // console.log(req.query, "|||", req.params);
  // query.query = qs.parse(query?.query)
  console.log(query, "Query 转化");
  if (query["query"]) {
    query.query = qs.parse(query?.query)
    console.log(query,"zhenshi query");
  ...
  if (query?.query?.query?.search) {
    console.log("query Set search");
    // query.query.query = {query.query.query}
    let obj = {
      $or: [
        {
          "title": {
            "$regex": new RegExp(query.query.query.search, "i")
          }
        },
        {
          "content": {
    ...
    delete query.query.query.search
    query.query.query = {...obj,...query.query.query}
  ...
  console.log(req.body, "req.body look");
