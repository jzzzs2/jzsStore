...
- 我们可以访问其官方网站对其进行学习: [router.vuejs.org/zh/](https://router.vuejs.org/zh/) vue-router是基于路由和组件的
...
import router from './router' //导入router 模块

Vue.config.productionTip = false

new Vue({
  store,
  router, // 创建和挂载根实例  router 配置参数注入路由，从而让整个应用都有路由功
  render: h => h(App)
}).$mount('#app')
...
- 默认值: `"router-link-active"`
  ...
  全局配置 `` 默认的激活的 class。参考 [router-link](https://router.vuejs.org/zh/api/#router-link)。
