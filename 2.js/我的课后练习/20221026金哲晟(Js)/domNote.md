...
+ 一个字符串 \\ ,一个是正则的 \\,正则中,对关键字匹配的需要加上 反斜杠 \,   \\,\/,\(,\)等等

  + 例子: 用字符串构建一个正则表达式  ( /^\d{4}[:\\\/\-]\d{2}[:\\\/\-]\d{2}$/ )
  + 例子: 用正则匹配反斜杠
...
+ Reflect.ownKeys(arr) //返回对象属性集合数组,包括不可遍历的length属性和Symal值属性

## Symbol 第七大数据类型

>1. Symbol值通过Symbol函数生成(不使用new 因为不是object),有了Symbol数据类型,对象的key值可以不仅仅是字符串了.
>
>2. Symbol值具有唯一性,不会出现属性名冲突.
...
+ 如果描述值是一个对象,会调用对象的toString方法,转化为字符串,再生成Symbol值.
+ Symbol值不能与其他类型值进行运算,但是可以转化为布尔类型值true

> Symbol值的属性 description
>
> ```
> let s = Symbol("hehe");
> s.description // "hehe"
...
> Symbol.for(描述值)  Symbol.keyFor(symbol值)
>
> ```
> Symbol.for(描述值) 会先在全局寻找描述值===xx的Symbol值,如果没有,在全局创建并注册,返回它.有的话,返回它
> Symbol.keyFor(symbol值) 获得一个全局Symbol值的描述值
> ```

## promise 初识

> 1. Promise是一个封装异步操作(回调函数,定时器等等)的容器,有两种状态,只有异步操作的结果可以决定他的状态.
> 2. Promise对象新建后就会立即执行.
> 3. catch和then同时书写了异步操作失败的回调函数时,如果异步操作失败,对应的回调函数就都会两次
> 4.  .then,.catch,.final  .final不论promise对象是什么状态都会执行.不带参数
...
>
>    ```
>    json对象(key必须是字符串 value是字符串/数值/布尔/嵌套): {
>    字符串: 字符串/数值/布尔/嵌套json对象
>    }
>    json数组(key是索引 value是字符串/数值/布尔/嵌套): [1,"str",true, {
>     "name": "jzs"
>    }]
>    ```
>
