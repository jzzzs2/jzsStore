 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2023-02-12 13:43:05
 */
const pagination = require("mongoose-sex-page")
async function pagiUtil (model,{size = 100,page = 1,display = 10} = {},link={}) {
  let result = await pagination(model).find().populate(link.key,link.value).select("+password").page(page).size(size).display(display).exec()
