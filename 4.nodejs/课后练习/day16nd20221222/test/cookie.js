 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2022-12-22 17:52:53
 */
let fs = require("fs")
...
http.createServer(function (req, res) {
  ...
  res.setHeader('content-type', 'text/html;charset=utf8')
  if (method === "GET" && /main/.test(urlName)) {
    let reqCookie = req.headers["cookie"]
    console.log(reqCookie, "cookie");
    if (/token=\d{5,}/.test(reqCookie)) {
      console.log("写入主页");
      // res.setHeader("Set-Cookie","token=qaq123")
      fs.readFile('./main.html', "utf8", function (err, data) {
        // let date = new Date()
        // date.setDate(date.getDate() + 1)
        // res.setHeader("Set-Cookie",`token=qaq123; Expires=${date.toUTCString()};HttpOnly`)
        // res.setHeader("Set-Cookie",`token=qaq123`)
        console.log(data);
        res.write(data)
        res.end()
    ...
    res.setHeader("Set-Cookie", `token=123342; Expires=${date.toUTCString()};HttpOnly`)
    fs.readFile("./login.html", "utf8", function (err, data) {
      console.log("跳转登录页", data);
      res.write(data)
      res.end()
    })
    return false
