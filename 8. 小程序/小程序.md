...
```
项目一键运行到微信开发者工具设置:
1)manifest.json项目配置文件中 > 微信小程序配置 > 输入你的appId
2)配置开发者工具启动路径: 工具 > 设置 > 运行配置
3)微信开发者工具端 允许操作 设置 > 安全设置 > 安全 > 服务端口 打开
4)Hbuilding 运行 > 运行到小程序
```
...
```
在components文件夹下创建,然后即可在所有位置用标签使用该组件
```
...
uni-icons
uni-tag
uni-search-bar 可修改源码,实现默认聚焦 {show:true,showsync:true}
```
